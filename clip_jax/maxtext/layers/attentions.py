...
class AttentionOp(nn.Module):
    ...
    def cudnn_flash_attention(self, query: Array, key: Array, value: Array) -> Array:
        ...
        if not has_fused_attn_kernel:
            raise ValueError(
                "Flash attention is not supported for current config i.e. head_dim, seq_len, n_heads etc."
    ...
    def update_ar_key_value(
        ...
        if self.quantize_kvcache:
            ...
            ar_value = quantizations.unquantize_kv(
                cached_value_var.value, cached_value_scale_var.value, one_token_value.dtype
            )
    ...
    def kv_cache_autoregressive(
        ...
        cache_ar_index.value = jnp.mod(
            cache_ar_index.value + 1, self.max_target_length - self.max_prefill_predict_length
        )
