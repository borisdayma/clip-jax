...
class AttentionOp(nn.Module):
    ...
    def apply_attention(
        self, query: Array, key: Array, value: Array, decoder_segment_ids: Array | None, model_mode: str
    ):
    ...
    def tpu_flash_attention(self, query: Array, key: Array, value: Array, decoder_segment_ids: Array | None) -> Array:
        ...
        def wrap_flash_attention(query, key, value, decoder_segment_ids):
            ...
            masks = [
                splash_attention_mask.CausalMask(shape=(query.shape[2], query.shape[2])) for i in range(query.shape[1])
            ]
    ...
    def compute_local_attention(
        self, attn_weights: Array, value: Array, q_seq_len: int, model_mode: str
    ...
    def _get_prefill_cache_vars(self, batch, heads, kv_head_size):
        ...
        if self.kv_quant:
            ...
            cache_scale_axis_names = self.transpose_tuple(
                self.cache_scale_logical_axis_names, self.prefill_cache_axis_order
            )
    ...
    def _get_ar_cache_vars(self, batch, heads, kv_head_size):
        ...
        if self.kv_quant:
            cache_scale_logical_shape = self._get_cache_scale_logical_shape(batch, heads)
            cache_scale_axis_names = self.transpose_tuple(
                self.cache_scale_logical_axis_names, self.ar_cache_axis_order
            )
    ...
    def kv_cache_prefill(
        ...
        cached_prefill_key_vars, cached_prefill_value_vars, cached_prefill_segment_id_var = (
            self._get_prefill_cache_vars(batch, heads, kv_head_size)
        )
    ...
    def update_ar_key_value(
        ...
        if self.kv_quant:
            ar_cache_scale_axis_names = self.transpose_tuple(
                self.cache_scale_logical_axis_names, self.ar_cache_axis_order
            )
    ...
    def kv_cache_autoregressive(
        ...
        cached_ar_key_vars, cached_ar_value_vars, cached_ar_segment_id_var, cache_ar_index_var = (
