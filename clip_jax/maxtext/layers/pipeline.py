...
class Pipeline(nn.Module):
    ...
    def setup(self):
        ...
        self.use_circ_storage = (
            self.config.num_pipeline_repeats > 1 and self.config.num_pipeline_microbatches > self.num_stages
        )
    ...
    def get_iteration_inputs(self, loop_iteration, state_io, circ_storage, shift):
        ...
        first_stage_in = jnp.where(
            loop_iteration < self.config.num_pipeline_microbatches, state_io_slice, circular_stage_in
        )
    ...
    def vmap_parallel_gather(self, weights, repeat_ids, repeat_dim_in_weights, stages_dim_in_weights):
        ...
        def _gather_one(x, repeat_id):
            return jnp.squeeze(
                jax.lax.dynamic_slice_in_dim(x, repeat_id, 1, repeat_dim_in_weights), repeat_dim_in_weights
            )
        ...
        * circ_storage_mover: assigned to rotated output and pushed into circ_storage on the next iteration
        ...
        if self.use_circ_storage:
            ...
            def _rotate_right_and_update(circ_storage_mover_in, circ_storage_in):
                ...
                offset = (
                    loop_iteration - (self.num_stages - 1) - 1
    ...
    def permute_output_micro_per_stage_dim(self, output):
        ...
        first_output_num_iters = (
            self.config.num_pipeline_microbatches * (self.config.num_pipeline_repeats - 1) + self.num_stages - 1
        )
        ...
        permutation = (
            np.arange(self.microbatches_per_stage) + land_idx
    ...
    def get_main_vmap_func(self):
        def func_to_vmap(
            body_instance, stages_inputs, stages_segment_ids, stages_positions, deterministic, model_mode
        ):
    ...
    def __call__(
        ...
        example_inputs = jax.lax.broadcast(
            inputs[0], [self.num_stages]
        )  # dummy inputs fed to initialize the module weights.
        if positions is not None:
            positions = positions.reshape(
                (self.config.num_pipeline_microbatches, self.microbatch_size, self.config.max_target_length)
            )
        ...
        total_iterations = (
            self.config.num_pipeline_microbatches * self.config.num_pipeline_repeats + self.num_stages - 1
        )
        ...
        if self.is_initializing():
            ...
            broadcasted_stage_outpus = jax.lax.broadcast(
                stage_outputs[0], [self.config.global_batch_size_to_train_on // self.microbatch_size]
            )
            return jnp.reshape(
                broadcasted_stage_outpus,
                [self.config.global_batch_size_to_train_on, self.config.max_target_length, self.config.emb_dim],
            )
        ...
        def run_iteration_scannable(model, loop_state, xs):
            ...
            return (
                model.run_one_iteration(loop_state, positions, segment_ids, deterministic, model_mode, model.layers),
                None,
            )
