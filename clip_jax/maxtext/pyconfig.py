...
from . import accelerator_to_spec_map
...
class _HyperParameters:
        ...
        for environment_var in os.environ:
            if environment_var[: len(_MAX_PREFIX)] == _MAX_PREFIX:
                ...
                if proposed_key not in raw_data_from_yaml:
                    raise ValueError(
                        f"We received env `{environment_var}` but it doesn't match a key, so it is assumed a mistake."
                    )
    ...
    def user_init(raw_keys):
        ...
        emb_scale, num_head_scale, mlp_dim_scale, layer_scale = get_individual_scales(
            raw_keys["global_parameter_scale"]
        )
        ...
        if using_pipeline_parallelism(raw_keys):
            ...
            assert (
                raw_keys["global_batch_size_to_train_on"] % raw_keys["num_pipeline_microbatches"] == 0
            ), f"The global batch size ({raw_keys['global_batch_size_to_train_on']}) must be divisible by the number of microbatches ({raw_keys['num_pipeline_microbatches']})"
        else:
...
def validate_megablox_parallelism(raw_keys):
    if raw_keys["megablox"] and (
        using_tensor_parallelism(raw_keys)
        or using_sequence_parallelism(raw_keys)
        or using_pipeline_parallelism(raw_keys)
    ):
...
def create_new_logical_axis_rules(old_logical_axis_rules, new_logical_axis_rules):
    ...
    old_logical_rules_filtered = [
        (old_logical_axis, old_mesh_axes)
