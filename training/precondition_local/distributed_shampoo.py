...
def preconditioning_compute_steps_schedule(
    ...
    preconditioning_compute_steps_t = (
        start_preconditioning_compute_steps + (1 - decay_factor) * end_preconditioning_compute_steps
    )
...
InversePthRootDiagnosticsSubtype = TypeVar("InversePthRootDiagnosticsSubtype", bound="InversePthRootDiagnostics")
...
class InversePthRootDiagnostics:
    ...
    @classmethod
    def create(
        cls: type[InversePthRootDiagnosticsSubtype],
        pth_inverse_root: jnp.ndarray,
        matrix: jnp.ndarray,
        p: Union[jnp.ndarray, int],
    ) -> InversePthRootDiagnosticsSubtype:
        """Generates a diagnostics struct from (-1/p) root result."""
        mat_m = jnp.matmul(mat_power(pth_inverse_root, p), matrix, precision=jax.lax.Precision.HIGHEST)
        ...
        return cls(
            ...
            avg_off_diag_error=(jnp.sum(off_diag_error) / num_off_diag_entries).astype(jnp.float32),
            p=jnp.array(p, jnp.float32),
        )


LOBPCGDiagnosticsSubtype = TypeVar("LOBPCGDiagnosticsSubtype", bound="LOBPCGDiagnostics")
...
class LOBPCGDiagnostics:
    ...
    @classmethod
    def create(
        cls: type[LOBPCGDiagnosticsSubtype],
        matrix: jnp.ndarray,
        eigvals: jnp.ndarray,
        eigvecs: jnp.ndarray,
        lobpcg_iters: jnp.ndarray,
    ) -> LOBPCGDiagnosticsSubtype:
        ...
        consistency_error_unnormalized = jnp.linalg.norm(mat_eigvecs - eigvals * eigvecs, ord=2, axis=0)
        ...
        return cls(
            ...
            avg_orthogonality_error=(jnp.sum(orthogonality_error) / num_off_diag).astype(jnp.float32),
...
class FDDiagnostics:
    ...
    def create(
        ...
        new_grad_sparsity = jnp.sum(mask * (new_grad == 0)).astype(jnp.float32) / padding_start**2
        new_grad_col_l1 = jnp.abs(new_grad).sum(axis=0)
        new_grad_col_sparsity = jnp.sum(nonpadding_ix * (new_grad_col_l1 == 0)).astype(jnp.float32) / padding_start
...
class TrainingMetrics:
    ...
    lobpcg_diagnostics: LOBPCGDiagnostics = struct.field(default_factory=LOBPCGDiagnostics)
    # Rich matrix entrywise error diagnostics, if enabled.
    inverse_pth_root_diagnostics: InversePthRootDiagnostics = struct.field(default_factory=InversePthRootDiagnostics)
    ...
    fd: Union[FDDiagnostics, optax.MaskedNode] = struct.field(default_factory=optax.MaskedNode)
...
def init_avg_grad(param: chex.Array, frequent_directions: bool) -> Union[chex.Array, optax.MaskedNode]:
...
def init_avg_grad_shape(param: chex.Array, frequent_directions: bool) -> Union[Any, optax.MaskedNode]:
...
def init_avg_grad_pspec(param: jax.sharding.PartitionSpec, frequent_directions: bool) -> Union[Any, optax.MaskedNode]:
    if not frequent_directions:
        return optax.MaskedNode()
    return param
...
def _low_rank_unpack(preconditioner, compression_rank):
    ...
    eigvecs, eigvals, inverted_eigvals, const, tail, has_zeros = _fd_low_rank_unpack(preconditioner, compression_rank)
    del eigvals, tail
...
def _low_rank_pack(eigvecs, eigvals, const, compression_rank):
    ...
    has_zeros = False
    return _fd_low_rank_pack(eigvecs, jnp.zeros_like(eigvals), eigvals, const, 0.0, has_zeros, compression_rank)
...
def _fd_low_rank_pack(eigvecs, deflated_eigs, inverted_eigs, new_const, new_tail, has_zeros, rank):
...
def _pth_root_difference(
    w: jnp.ndarray, alpha: jnp.ndarray, beta: jnp.ndarray, p: Union[int, jnp.ndarray]
) -> jnp.ndarray:
...
def matrix_inverse_pth_root(
    ...
    if eigh:
        return matrix_inverse_pth_root_eigh(
            matrix, p, ridge_epsilon, error_tolerance, precision, relative_matrix_epsilon, padding_start, prev
        )
    del prev
    ...
    if padding_start is not None:
        ...
        ix = (jnp.arange(matrix_size, dtype=jnp.int32) < padding_start).astype(matrix.dtype)
    ...
    if relative_matrix_epsilon:
        if eigvals is not None:
            ...
            _, max_ev = power_iteration(
                matrix=matrix, num_iters=100, error_tolerance=1e-6, precision=precision, padding_start=padding_start
            )
    else:
    ...
    def _iter_condition(state):
        ...
        error_above_threshold = jnp.logical_and(error > error_tolerance, error_ratio < max_error_ratio)
    ...
    if matrix_size == 1:
        ...
        def _outer_body_fn(state):
            ...
            init_state = tuple([0, new_mat_m_0, new_mat_h_0, new_mat_h_0, new_error, 1.0])
            ...
            return (i + 1, resultant_mat_h, error, iters, error_ratio, error > retry_loop_error_threshold)
    ...
    if lobpcg_topk_precondition > 0:
        ...
        conditioned_diagnostics = InversePthRootDiagnostics.create(conditioned_resultant_mat, damped_matrix, p)
        unconditioned_damped_matrix = original_matrix + ridge_epsilon * identity
        unconditioned_diagnostics = InversePthRootDiagnostics.create(resultant_mat_h, unconditioned_damped_matrix, p)
        ...
        unconditional_errors = jnp.maximum(
            unconditioned_diagnostics.max_diag_error, unconditioned_diagnostics.max_off_diag_error
        )
    ...
    if padding_start is not None:
        ...
        error = jnp.where(padding_start == 0, 0.0, error_metrics.inverse_pth_root_errors)
...
def matrix_inverse_pth_root_eigh(
    ...
    inv_e = jnp.where(e == 0.0, 0.0, jnp.power(jnp.maximum(e, ridge_epsilon), alpha))
    ...
    error_metrics = TrainingMetrics(inverse_pth_root_errors=jnp.array(error, jnp.float32))
    if padding_start is not None:
...
def _low_rank_root(
    ...
    const = jnp.sum(to_avg_e) / jnp.where(num_real_eigs_to_avg > 0, num_real_eigs_to_avg, 1.0)
...
def _fd_update_root(
    ...
    sketch_dr, fwd_eigvals_r, inv_eigvals_r, const, tail, had_zeros = _fd_low_rank_unpack(prev, rank)
    ...
    num_unsafe_norms = jnp.sum(~safe_normed) - (num_neg_eigs + num_zero_deflated_eigs)
    ...
    recovered = jnp.matmul((u[:, :rank] * s[jnp.newaxis, :rank]), vt[:rank, :], precision=jax.lax.Precision.HIGHEST)
...
def gram_weighted_update(
    old_stats: chex.Array, g: chex.Array, axis: int, w1: float, w2: float, precision: Optional[lax.Precision] = None
) -> chex.Array:
...
def frequent_directions_update(
    old_stats_factor: chex.Array, g: chex.Array, axis: int, w1: float, w2: float
) -> chex.Array:
...
class Preconditioner:
    ...
    def updated_statistics_from_grad(
        ...
        preconditioned_dims = [i for i, p in enumerate(should_preconditioned_dims) if p]
        new_stats = []
        index = 0
    ...
    def _precondition_block_shampoo(self, g, should_precondition_dim, preconditioners):
        ...
        for j, should_precondition in enumerate(should_precondition_dim):
            ...
            if compress:
                eigvecs, eigvals, const, skip = _low_rank_unpack(preconditioners[j], abs(self._compression_rank))
                assert list(eigvecs.shape) == [dim, abs(self._compression_rank)]
                lowrank_basis = jnp.tensordot(g, eigvecs, axes=[[0], [0]])
                lowrank_component = jnp.tensordot(lowrank_basis, eigvecs, axes=[[rank - 1], [1]])
                g = jnp.transpose(g, axes=roll)
                complement = g - lowrank_component
                scaled_basis = lowrank_basis * eigvals
                scaled_lowrank_component = jnp.tensordot(scaled_basis, eigvecs, axes=[[rank - 1], [1]])
                old_g = g
...
def _update_preconditioners_fn(
    update_preconditioners_every_fn, update_preconditioners_fn, steps, scheduled, quantized=False
):
    ...
    if quantized:
        if scheduled:
            (
                ...
                metrics_flat,
            ) = lax.cond(steps == 1, update_preconditioners_every_fn, update_preconditioners_fn)
        else:
            if steps == 1:
                (
                    quantized_preconditioners_flat,
...
def _convert_to_parameter_stats(global_stats, local_stat, compression_rank, convert_statistics=True):
...
def _add_metrics_into_local_stats(local_stats, metrics, keep_old):
    ...
    for local_stat in local_stats:
        ...
        # values).
        per_stat_metrics = efficient_cond(keep_old, lambda: [local_stat.training_metrics], [per_stat_metrics])[0]
...
def distributed_shampoo(
    ...
    if frequent_directions and compression_rank <= 0:
        raise ValueError("frequent_directions=True requires compression_rank > 0," f" found {compression_rank}")

    if average_grad and not frequent_directions:
        raise ValueError("average_grad requested but frequent_directions is False")

    if frequent_directions and (statistics_compute_steps != preconditioning_compute_steps):
        raise ValueError(
            "frequent_directions=True requires "
    ...
    def _graft_type_has_diagonal_statistics():
        ...
        return graft_type not in [GraftingType.SGD, GraftingType.SQRT_N, GraftingType.NONE]
    ...
    quantize_second_moment = (
        best_effort_memory_usage_reduction and not compression_rank and not frequent_directions and batch_axis_name
    )
    ...
    def pad_and_maybe_zero_preconditioners(preconditioners, total, max_size, step):
    ...
    def sharded_init_fn(params):
        ...
        for param in params_flat:
            ...
            if not _skip_preconditioning(param):
                ...
                preconditioners = [jnp.eye(max_size, pd, dtype=jnp.float32) * (pd == max_size) for s in shapes]
        ...
        padded_preconditioners.extend(
            [jnp.eye(max_size, pd, dtype=stat_dtype) * (pd == max_size) for _ in range(to_pad)]
        )
    ...
    def sharded_init_partition_spec_fn(params, params_partition_spec, partition_spec_for_statistics):
        ...
        param_pspec_flat, _ = jax.tree_util.tree_flatten(params_partition_spec, is_leaf=lambda x: x is None)
        params_flat, treedef = jax.tree_util.tree_flatten(params)
        ...
        for param, param_pspec in zip(params_flat, param_pspec_flat):
            ...
            local_stats_flat.append(
                LocalShardedParameterStats(  # pytype: disable=wrong-arg-types  # numpy-scalars
                    ...
                    init_avg_grad_pspec(param_pspec, frequent_directions and average_grad),
        ...
        local_stats = jax.tree_util.tree_unflatten(treedef, local_stats_flat)
        global_stats = GlobalShardedParameterStats(
            partition_spec_for_statistics,  # pytype: disable=wrong-arg-types  # numpy-scalars
    ...
    def sharded_init_shape_and_dtype_fn(params):
            ...
            local_stats_flat.append(
                LocalShardedParameterStats(  # pytype: disable=wrong-arg-types  # numpy-scalars
                    ...
                    QuantizedValue(
                        m1_shape_and_dtype,
                        [],
                        m1_scale_shape_and_dtype,  # pytype: disable=wrong-arg-types  # numpy-scalars
                        qdtype,
                        False,
                        list(param.shape),
                    ),
                    QuantizedValue(
                        m2_shape_and_dtype,
                        [],
                        m2_scale_shape_and_dtype,  # pytype: disable=wrong-arg-types  # numpy-scalars
                        qdtype,
                        False,
                        list(param.shape),
                    ),
        ...
        preconditioners_shape = [num_statistics, max_statistics_size, precond_dim(max_statistics_size)]
        global_stats = GlobalShardedParameterStats(  # pytype: disable=wrong-arg-types  # numpy-scalars
            [statistics_shape, jnp.float32], [preconditioners_shape, jnp.float32], [[num_statistics], jnp.int32]
        )
        return ShampooState(  # pytype: disable=wrong-arg-types  # numpy-scalars
            count=[[], jnp.float32], stats=ShardedShampooStats(global_stats, local_stats)
        )
    ...
    def sharded_update_fn(grads, state, params):
        ...
        new_stacked_padded_statistics = jax.lax.with_sharding_constraint(
            new_stacked_padded_statistics, statistics_partition_spec
        )
        stacked_padding_starts = jnp.array(padding_starts, jnp.int32)
        prev_stacked_padded_preconditioners = _maybe(jnp.stack)(prev_padded_preconditioners)
        prev_stacked_padded_preconditioners = _maybe(jax.lax.with_sharding_constraint)(
        ...
        scheduled_preconditioning_compute_steps = (
            decay_preconditioning_compute_steps and end_preconditioning_compute_steps and callable(learning_rate)
        )
        ...
        def _update_preconditioners():
            ...
            new_errors = jnp.ones_like(metrics_init.inverse_pth_root_errors) * (inverse_failure_threshold)
            metrics_init = metrics_init.replace(inverse_pth_root_errors=new_errors)
            init_state = [preconditioners_init, metrics_init]
            return efficient_cond(perform_step, _internal_inverse_pth_root_all, init_state)
        ...
        if generate_training_metrics:
            new_local_stats_flat = _add_metrics_into_local_stats(new_local_stats_flat, metrics, ~perform_step)
    ...
    def init_fn(params):
        ...
        def _init(param):
            ...
            if not _skip_preconditioning(param):
                ...
                # If the preconditioner is using a low-rank representation, initialize
                # it to zero instead of an invalid eye.
                preconditioners = [jnp.eye(s[0], s[1], dtype=jnp.float32) * (s[0] == s[1]) for s in shapes]
        ...
        return ShampooState(count=jnp.zeros([], jnp.int32), stats=jax.tree.map(_init, params))
        ...
        if not _skip_preconditioning(param):
            if frequent_directions and average_grad:
                new_avg_grad = jnp.where(
                    jnp.logical_or(statistics_compute_steps == 1, step % statistics_compute_steps == 1),
                    grad,
                    state.avg_grad + grad,
                )
    ...
    if compression_rank != 0:
        ...
        def small_mi_pth_root(stats, exponents, padding_start, prev):
            ...
            root, metrics = original_mi_pth_root(stats, exponents, padding_start=padding_start, prev=prev)
    ...
    def _matrix_inverse_pth_root_vmap(xs, ps, padding_starts, prev):
        return jax.vmap(mi_pth_root)(xs, ps, padding_start=padding_starts, prev=prev)

    def _quantized_matrix_inverse_pth_root_vmap(qxs, qds, qbs, ps, padding_starts, qpxs=None, qpds=None, qpbs=None):
        ...
        def matrix_inverse_pth_root_wrapper(qx, qd, qb, p, padding_start, qpx, qpd, qpb):
            v = _quantized_to_float(qx, qd, qb)
            prev = _maybe(_quantized_to_float)(qpx, qpd, qpb)
            preconditioner, metrics = mi_pth_root(v, p, padding_start=padding_start, prev=prev)
            qp = QuantizedValue.from_float_value(preconditioner, qx.dtype, True)
            return qp.quantized, qp.diagonal, qp.bucket_size, metrics

        return jax.vmap(matrix_inverse_pth_root_wrapper)(qxs, qds, qbs, ps, padding_starts, qpxs, qpds, qpbs)

    def _matrix_inverse_pth_root_pjit(xs, ps, padding_starts, prev_preconds=None, statistics_partition_spec=None):
        # Partition the concatenated statistics matrix across all cores.
        pspec_for_partition = preconditioner_partition_spec
        partitioned_xs = jax.lax.with_sharding_constraint(xs, pspec_for_partition)
        if preconditioner_partition_spec:
            partitioned_ps_spec = jax.sharding.PartitionSpec(preconditioner_partition_spec[0])
        else:
            partitioned_ps_spec = None
        partitioned_ps = jax.lax.with_sharding_constraint(ps, partitioned_ps_spec)
        partitioned_prev_preconds = _maybe(jax.lax.with_sharding_constraint)(
            prev_preconds, preconditioner_partition_spec
        )
        partitioned_padding_starts = jax.lax.with_sharding_constraint(
            padding_starts, partitioned_ps_spec
        )  # paddings are scalars like ps.
        # Run matrix inverse pth root on each shard.
        partitioned_preconditioners, partitioned_metrics = _matrix_inverse_pth_root_vmap(
            partitioned_xs, partitioned_ps, partitioned_padding_starts, prev=partitioned_prev_preconds
        )
        # Reshard output to have the same PSpec as input. This is required to avoid
        # vmap seeing the full set of statistics.
        partitioned_preconditioners = jax.lax.with_sharding_constraint(
            partitioned_preconditioners, pspec_for_partition
        )
        # Recombine the outputs at each core.
        preconditioners = jax.lax.with_sharding_constraint(partitioned_preconditioners, statistics_partition_spec)
        metrics = jax.lax.with_sharding_constraint(partitioned_metrics, jax.sharding.PartitionSpec())
        return preconditioners, metrics
    ...
    def _pmap_compute_preconditioners(
        ...
        def _internal_inverse_pth_root_all():
            if batch_axis_name:
                ...
                metrics = jax.tree.map(functools.partial(jnp.expand_dims, axis=0), metrics)
                metrics_flat = jax.tree.map(unbatch, metrics)
        ...
        def _update_preconditioners():
            ...
            # shaped tensors. Note statistics will be ignored as we are passing in
            # a large error value.
            preconditioners_init = [s[:, : precond_dim(s.shape[0])] for s in packed_statistics]
            n = len(packed_statistics)
            metrics_init = jax.tree.map(
                lambda x: [x] * n,
                default_training_metrics(generate_fd_metrics).replace(
        ...
        assert len(new_errors_flat) == len(packed_statistics), (len(new_errors_flat), len(packed_statistics))
        assert len(new_errors_flat) == num_statistics + to_pad, (len(new_errors_flat), num_statistics, to_pad)
            ...
            if num_statistics == 0:
                ...
                if generate_training_metrics:
                    ...
                    assert jax.tree_util.tree_all(
                        jax.tree.map(lambda x: len(state.statistics) == len(x), metrics_for_state)
                    )
                ...
                idx += num_statistics
        new_states = []
        for state, new_preconditioners, new_metrics in zip(states, preconditioners_for_states, metrics_for_states):
    ...
    def _pmap_quantized_compute_preconditioners(
        ...
        all_exponents = batch(exponents, num_devices)
        all_paddings = batch(paddings, num_devices)
        all_quantized_precond_mats = _maybe(batch)(packed_quantized_precond_mats, num_devices)
        all_quantized_precond_diagonals = _maybe(batch)(packed_quantized_precond_diagonals, num_devices)
        all_quantized_precond_bucket_sizes = _maybe(batch)(packed_quantized_precond_bucket_sizes, num_devices)
        ...
        def _update_quantized_preconditioners():
            ...
            # a large error value.
            pd = precond_dim(max_size)
            quantized_preconditioners_init = [s[:, :pd] for s in packed_quantized_statistics]
    ...
    def _pjit_compute_preconditioners(
        ...
        def _update_preconditioners():
            ...
            metrics_init = jax.tree.map(
                lambda x: [x] * n, TrainingMetrics(inverse_pth_root_errors=inverse_failure_threshold)
            )
            init_state = [preconditioners_init, metrics_init]
            perform_step = step % preconditioning_compute_steps_t == 0
            return efficient_cond(perform_step, _internal_inverse_pth_root_all, init_state)
        ...
        for p, shape, prev_p, error in zip(
            ...
            new_preconditioners_flat.append(
                _select_preconditioner(error.inverse_pth_root_errors, p[: shape[0], : shape[1]], prev_p)
            )
    ...
    def _transform_grad(grad, state, param, step):
        ...
        if not _skip_preconditioning(param):
            ...
            if graft_type == GraftingType.NONE:
                logging.error("skipping preconditioning without grafting for param %s", param)
        ...
        if weight_decay != 0 and decoupled_weight_decay:
            ...
            nesterov_momentum_update = nesterov_momentum_update + wd_lr * weight_decay * param
